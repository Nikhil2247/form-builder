...
describe('hiddenByLegacyLogic', () => {
  ...
  it('hides a SHOW target until its condition is met', () => {
    const rules = [rule({})];
    expect(hiddenByLegacyLogic(questions, rules, {}).has('q_target')).toBe(
      true,
    );
    expect(
      hiddenByLegacyLogic(questions, rules, { q_trigger: 'yes' }).has(
        'q_target',
      ),
    ).toBe(false);
  });
  ...
  it("matches a numeric answer against the rule's string value", () => {
    ...
    expect(
      hiddenByLegacyLogic(questions, rules, { q_trigger: 7 }).has('q_target'),
    ).toBe(false);
    expect(
      hiddenByLegacyLogic(questions, rules, { q_trigger: 8 }).has('q_target'),
    ).toBe(true);
  });
  ...
  it('matches any member of a multi-choice answer', () => {
    ...
    expect(
      hiddenByLegacyLogic(questions, rules, { q_trigger: ['a', 'b'] }).has(
        'q_target',
      ),
    ).toBe(false);
    expect(
      hiddenByLegacyLogic(questions, rules, { q_trigger: ['a'] }).has(
        'q_target',
      ),
    ).toBe(true);
  });
  ...
  it('treats 0 as an answer for IS_FILLED', () => {
    ...
    expect(
      hiddenByLegacyLogic(questions, rules, { q_trigger: 0 }).has('q_target'),
    ).toBe(false);
    expect(
      hiddenByLegacyLogic(questions, rules, { q_trigger: '' }).has('q_target'),
    ).toBe(true);
  });
  ...
  it('compares numerically for GREATER_THAN / LESS_THAN', () => {
    ...
    expect(
      hiddenByLegacyLogic(questions, gt, { q_trigger: '6' }).has('q_target'),
    ).toBe(false);
    expect(
      hiddenByLegacyLogic(questions, gt, { q_trigger: '5' }).has('q_target'),
    ).toBe(true);

    const lt = [rule({ operator: 'LESS_THAN', value: '5' })];
    expect(
      hiddenByLegacyLogic(questions, lt, { q_trigger: 4 }).has('q_target'),
    ).toBe(false);
  });
  ...
  it('ignores JUMP_TO_PAGE rules, which target no question', () => {
    const rules = [
      rule({ action: 'JUMP_TO_PAGE', targetQuestionId: undefined }),
    ];
