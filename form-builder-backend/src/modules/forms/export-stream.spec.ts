    ...
    reader: {
      ...
      formSubmission: {
        ...
        findMany: jest.fn(({ take, cursor, skip }: any) => {
          const start = cursor
            ? rows.findIndex((r) => r.id === cursor.id) + (skip ?? 0)
            : 0;
...
describe('exportSubmissions (streamed)', () => {
  it('emits a header row and one line per submission, CRLF separated', async () => {
    ...
    const csv = await collect(
      await service(prismaWith(rows)).exportSubmissions('org', 'form', 'csv'),
    );

    const lines = csv.split('\r\n');
    expect(lines).toHaveLength(3);
    expect(lines[0]).toBe(
      '"Submission ID","Submitted At","Status","Country","Full name","Rating","q3"',
    );
    expect(lines[1]).toBe(
  ...
  it('produces a parseable JSON array', async () => {
    ...
    const json = await collect(
      await service(prismaWith(rows)).exportSubmissions('org', 'form', 'json'),
    );
  ...
  it('is an empty JSON array when there is nothing to export', async () => {
    expect(
      await collect(
        await service(prismaWith([])).exportSubmissions('org', 'form', 'json'),
      ),
    ).toBe('[]');
  });
  ...
  it('throws for an unknown form before yielding anything', async () => {
    await expect(
      service(prismaWith([], null)).exportSubmissions('org', 'nope', 'csv'),
  ...
  it('throws when the row count exceeds the cap', async () => {
    ...
    await expect(
      service(prisma).exportSubmissions('org', 'form', 'csv'),
