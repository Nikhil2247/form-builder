...
import { Shield, Building2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function GlobalAuditPage() {
  ...
  const { data, isLoading } = useGlobalAudit(page);
  const logs = (data as any)?.data ?? (data as any)?.logs ?? [];
    ...
    <div className="w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Platform-wide activity across all organizations.</p>
        </div>
        ...
        <div className="divide-y divide-border">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <Shield size={24} className="mb-3 text-muted-foreground" />
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {log.organization && (
                        <span className="flex items-center gap-1 text-xs text-muted-foreground">
                          <Building2 size={11} />{log.organization.name}
                        </span>
                      )}
                    </div>
